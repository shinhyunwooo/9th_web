...
const MovieDetailPage = () => {
  ...
  useEffect(() => {
    ...
    const headers = {
      Authorization: `Bearer ${import.meta.env.VITE_TMBD_KEY}`, // ✅ 통일
      "Content-Type": "application/json;charset=utf-8",
    };

    Promise.all([
      axios.get<MovieDetails>(`https://api.themoviedb.org/3/movie/${movieId}?language=ko-KR`, { headers, signal: ctrl.signal }),
      axios.get<Credits>(`https://api.themoviedb.org/3/movie/${movieId}/credits?language=ko-KR`, { headers, signal: ctrl.signal }),
    ])
      .then(([d, c]) => {
        setDetail(d.data);
        setCredits(c.data);
      ...
      .catch((e) => {
        ...
        setIsError(e?.message ?? "요청 중 오류가 발생했습니다.");
      })
        ...
        <div className="absolute bottom-4 left-4 flex items-end gap-4">
          ...
          <div>
            ...
            <p className="mt-1 text-sm opacity-80">
              {detail.release_date?.slice(0, 4)} · {detail.runtime ?? "?"}분 · ⭐ {detail.vote_average.toFixed(1)}
            </p>
